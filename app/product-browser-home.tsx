...
export default function ProductBrowserHome() {
  ...
  return <main className="min-h-dvh bg-[#f7f7f4] text-[#171717]">
    ...
    <section id="top" className="mx-auto max-w-7xl px-5 pb-10 pt-12 lg:px-8 lg:pt-16"><div className="product-hero-row"><div className="product-hero-copy max-w-3xl"><h1 className="text-4xl font-semibold leading-[1.08] tracking-[-0.065em] sm:text-5xl lg:text-[4rem]">每一个用 AI 创造的产品，<br /><span className="text-black/35">尤其是非专业创造者的产品，都有被发现的机会。</span></h1></div><div className="product-hero-logo-zone" aria-label="First Look 彩色标志" onMouseMove={(event) => { const bounds = event.currentTarget.getBoundingClientRect(); setLogoReveal(Math.max(0, Math.min(100, ((event.clientX - bounds.left) / bounds.width) * 100))); }} onMouseLeave={() => setLogoReveal(0)}><div className="product-hero-logo" style={{ clipPath: `inset(0 ${100 - logoReveal}% 0 0)` }}><span data-wordmark="FIRST LOOK">FIRST LOOK</span></div></div></div></section>
