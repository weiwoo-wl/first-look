...
type Product = { id:number; slug:string; title:string; description:string; type:string; status:string; visibility:string; updated_at:string; latest_version:number|null; likes_total:number };
...
export default function ProductsPage() {
  ...
  return <main className="min-h-dvh bg-[#f7f7f4] text-[#171717]">
    ...
    <section className="mx-auto max-w-4xl px-5 py-12">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-black/45">产品内容</p><h1 className="mt-2 text-4xl font-semibold tracking-[-.06em]">我的产品</h1><p className="mt-4 max-w-xl text-sm leading-6 text-black/55">从第一次发布到现在，每一次变化都会沿着生命线留下来。</p></div><a href="/account/creator" className="inline-flex min-h-11 items-center rounded-full border border-black/15 bg-white px-4 py-2 text-sm">管理公开主页</a></div>
