...
export default function ProductControls({ id, slug, title, visibility, onVisibilityChange }: { id:number; slug:string; title:string; visibility:string; onVisibilityChange:(next:string)=>void }) {
  ...
  return <div className="flex flex-wrap items-center gap-2"><Link href={`/account/products/${id}/edit`} className="inline-flex items-center gap-1 rounded-full border border-black/12 px-3 py-2 text-xs"><Edit3 size={14}/>编辑</Link>{visibility!=="draft"&&<button disabled={busy} onClick={()=>setVisibility(visibility==="published"?"unpublish":"republish")} className="rounded-full border border-black/12 px-3 py-2 text-xs disabled:opacity-50">{busy?"处理中…":visibility==="published"?"下架产品":"重新上架"}</button>}{visibility==="published"&&<Link href={`/work/${encodeURIComponent(slug)}`} aria-label={`查看 ${title}`} className="rounded-full border border-black/12 p-2"><ExternalLink size={15}/></Link>}{error&&<span role="alert" className="w-full text-xs text-red-600">{error}</span>}</div>;
}
